...
## 5. M2 재실행 — 후보 구현의 졸업 판정

M2에서 하네스의 이펙트 수학·드로잉을 스파이크 로컬 후보에서 **프로덕션
경로**(`domain/kvloop/effects` + `compositions/kvloop/KvEffectsCanvas` —
`KvScene`이 실제로 마운트하는 코드)로 교체하고 같은 게이트로 재실행했다.

**5/5 PASS, M0과 동일 수치** — ① 2회 렌더 sha256 비트 동일(디코드 차 0.000),
격리 띠 최대 0.080, 단독 드로잉 vs 인코딩 0.48/255, 글로우 중심 0.620→0.643
(예측 0.644). 워밍업 포함 첫 렌더를 제외한 비용 참고치는 a2 2.2초 vs off
1.5초(카메라 정지). 후보와 프로덕션이 수치까지 같으므로 이식 과정에서
동작이 변하지 않았다.
...
| 0.2.0 | 2026-08-26 | 김성권 / Claude | M2 재실행(§5) — 하네스를 프로덕션 코드 경로로 전환, 5/5 PASS 동일 수치 |
