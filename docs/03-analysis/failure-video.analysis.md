...
| 전환 아웃 0.2s 줌인 + 블러 | `TRANSITION_OUT_MS` 250 | 프레임 08 | 일치 |
| 전환 인 0.3s 줌아웃 안착 | `TRANSITION_IN_MS` 300 | 프레임 09~10 | 일치 |
...
`plain`의 −1%가 이 사이클이 지켜야 했던 규칙(Goal 4)의 증거다.
...
**FAIL 비트와 레벨 1의 아웃고잉 펀치가 시간상 겹친다.**

Design은 둘을 각각 정의한다: FAIL 창은 구간 1의 **마지막 1.5초**(D-3, 사용자 요구
"마지막 1초 고정"), 펀치 전환은 구간의 **마지막 0.25초**(§5.3). 30초 프리셋 기준
구간 1은 162프레임이고, FAIL 창은 프레임 117–161, 펀치 아웃은 154–161이다 —
**마지막 8프레임에서 둘 다 걸린다.** 영상 레이어에는 FAIL 줌 2.2배와 전환 줌
2.0배가 곱해져 순간 4.4배가 된다.

레퍼런스에서는 이 둘이 겹치지 않는다. 거기서는 FAIL이 9.6초 구간의 2.9–4.5초에
있고 전환은 9.4–10.0초라 서로 떨어져 있다. 우리는 사용자 요구에 따라 FAIL을 구간
**끝**에 못 박았으므로 구조적으로 붙는다.

실제로 찍어 보면(`artifacts/failure/frames/08-punch-out.png`) 안착한 스탬프째로
화면이 카메라 쪽으로 돌진하며 블러로 빠지는 그림이 되어 **깨져 보이지는 않는다.**
Design이 정의한 두 동작의 합성이므로 사양대로 구현했고, 판단만 남겨 둔다.

> **질문**: 이대로 둘까요, 아니면 펀치 전환을 FAIL 창 밖으로 밀까요(예: 전환을
> 스탬프 소멸 이후로 옮기거나, 구간 1에서만 전환을 생략)? 전자는 코드 변경 0,
> 후자는 `failureEdgesAt`에 한 조건입니다.
...
| FR-05 줌 펀치 전환, Σ불변 | `zoomPunchAt`, `failureEdgesAt` | 유닛 + M4 게이트 |
