...
- `zoomPunchAt(frameInSection, durationInFrames, fps, edges)` → `{scale, blurRatio} | null` —
  ...
  섹션별 edges: panel-a `{in:false, out:false}`, panel-b `{in:false, out:true}`,
  panel-c `{in:true, out:false}` — **엔드카드 경계는 컷** (day1과 동일, 레퍼런스에 근거 없음).

> **D-12 (2026-08-28, Do 중 사용자 결정) — 레벨 1은 펀치 아웃도 하지 않는다.**
> 초안은 panel-a `{in:false, out:true}`였다. 그런데 FAIL 창은 구간 1의 **끝**에
> 앵커돼 있어(D-3, 사용자 요구 "마지막 1초 고정") 둘이 겹친다: 30초 프리셋에서
> 구간 1은 162프레임, FAIL 창은 117–161, 펀치 아웃은 154–161 — **마지막 8프레임에
> 둘 다** 걸리고 영상 레이어는 2.2 × 2.0 = 4.4배가 된다. 레퍼런스에는 이 상태가
> 없다(거기서는 FAIL이 구간 중간, 전환은 수 초 뒤다). Do 단계에서 실측으로
> 확인해 사용자에게 물었고, **레벨 1 → 레벨 20 경계를 컷으로** 하기로 했다.
> 전환의 나머지 절반(panel-b의 in)도 함께 뺀다 — 앞선 줌인 없이 줌아웃만 남으면
> 2배에서 튀어나오는 것처럼 보인다. 결과적으로 영상에 남는 펀치는 레벨 20 →
> 레벨 99 하나이고, 이는 레퍼런스의 두 번째 전환에 대응한다.
...
| D-12 | **레벨 1 → 레벨 20 경계는 컷** — FAIL 창이 구간 끝 앵커라 펀치와 마지막 8프레임이 겹쳐 영상 줌이 4.4배가 됐다. Do 중 실측 후 사용자 결정 (2026-08-28) | §6.2 |
...
| 2026-08-28 | Do 중 D-12 추가 — 레벨 1의 펀치 아웃을 뺀다(사용자 결정). §6.2 `failureEdgesAt` 갱신 |
