...
> **Author**: 김성권 / Claude
> **Date**: 2026-08-27
> **Status**: **Confirmed** — §12의 확인 항목이 닫혔다(2026-08-27). **M1 착수 가능**
...
### 12.1 확인 결과 — 2026-08-27

요청자와 항목별 확인. **1·2·3은 설계의 선택 그대로**, 4는 예정대로 M0으로 남는다.

| # | 결정 | 이 결정이 M1에서 하는 일 |
|---|---|---|
| 1 | 글로우의 `center`·`radius`가 **`region` 안으로** 들어간다 | `region`이 `rect \| circle \| mask` 유니온이 되고, `effect.center`·`effect.radius`를 읽는 네 곳이 `effect.region.…`으로 바뀐다(§10의 M1 각주). 값·그리기 인수·렌더 결과는 동일 |
| 2 | 자동 후보는 **마스크로 승격**된다 | `lightRegions`가 박스가 아니라 셀 목록을 내고, 승인이 `shape: 'mask'` 오브젝트를 만든다. 모델 없이도 마스크 지정이 되는 경로가 하나 생긴다 |
| 3 | 여유값·클릭 지점을 **저장하지 않는다** | 스키마가 마스크 하나로 끝난다. 재조절은 새 지정 세션(추론 다시)이고, "모델은 렌더 입력이 아니다"가 스키마에서도 깨끗하게 읽힌다 |
| 4 | 임계값 기본값은 **M0에서 확정** | 0.3을 잠정 상수로 두고 M0의 실측값으로 교체한다 — 이전 사이클 M4(기본값 교정)와 같은 절차 |

1번의 귀결을 한 번 더 적어 둔다: **유니온의 축이 하나뿐이므로**, 이후 이펙트
종류가 늘어도(라이트 스윕 등) 형태가 재생산되지 않는다. 이것이 D-A05가 지난
사이클 Design의 "마스크는 새 `kind`" 가정을 뒤집은 이유이고, 지금 글로우를
예외로 두면 그 이득을 절반만 받는다.
...
|---------|------|--------|--------|
| 0.2.0 | 2026-08-27 | 김성권 / Claude | §12 확인 완료(§12.1) — 글로우 필드의 region 편입·자동 후보의 마스크 승격·지정 이력 미저장 확정. Confirmed 승격, M1 착수 가능 |
