# day1-label-effects Do Report

> 2026-08-26 (단일 세션) · Plan → (경량 Design, Plan §5 내장) → Do → Check(부분)
> 브랜치 `claude/label-textbox-effect-toggle-5kayu6` · **main 병합 보류**(사용자 요청: 다른 기능 진행 중)
> 상태: **구현 완료, SC5(실렌더 픽셀)만 이 컨테이너에서 실행 불가 — §4 참조**

발단은 기능 요청이 아니라 **실현 가능성 질문**이었다. "Day1 · Day1(4 video) 라벨에
텍스트 박스 on/off를, 가능하면 글로우 on/off까지"라는 요청에 대해 Plan §0에서 두
템플릿이 `Panel.tsx`의 `PanelLabel` 하나를 공유한다는 것과, 두 효과가 이미 다른
오버레이에서 같은 렌더 경로로 MP4에 구워지고 있다는 것을 코드 근거로 확인한 뒤,
Q1–Q5 확정을 받아 그대로 구현했다.

## Executive Summary

| 항목 | 값 |
|------|-----|
| Feature | day1-label-effects |
| 변경 규모 | 프로덕션 7파일(신규 1) · 테스트 4파일(신규 1) — `labelStyle` 6필드, 컴포지션 1곳, 인스펙터 2토글+4필드 |
| 최종 검증 | 유닛 **588 passed**(신규 7) · `tsc -b` + `vite build` 성공 · 신규 e2e L2 **2 passed** · 기존 e2e 회귀 7 passed |
| 미실행 | 신규 e2e L3(실렌더 2건) — 이 컨테이너에 H.264 코덱을 가진 Chrome이 없음(§4) |
| 착수 시 기준선 | 유닛 581 passed |

### Value Delivered

| 관점 | 계획 | 실제 전달 |
|------|------|-----------|
| Problem | 밝은 게임플레이 위 라벨 가독성 부족 | 라벨 뒤 반투명 채움 판 + 글자 둘레 글로우, 각각 독립 on/off |
| Solution | 공유 `labelStyle`에 6필드 | 스키마·기본값·클램프·렌더·UI 전부 반영, 신규 커맨드 0개 |
| Function UX | 두 템플릿 동시 적용 | `Panel.tsx` 1곳 수정으로 Day1·Day1(4 video) 동시 적용(유닛 테스트로 양쪽 고정) |
| Core Value | 기존 프로젝트 무영향 | zod `.default()` + 기본 꺼짐 — 마이그레이션 0줄, `PROJECT_SCHEMA_VERSION` 2 유지 |

## 1. Key Decisions & Outcomes

| 결정 | 근거 | 결과 |
|------|------|------|
| Q1 채움 판(테두리 사각형 아님) | 자막 "배경 사용"과 같은 개념, 가독성 목적에 직접 부합 | `SubtitleOverlay`와 동일한 여백(`0.3em 0.6em`)·모서리(8px) — 새 상수 0개 |
| Q2·Q3 색·수치까지 노출 | 흰 글자에 흰 글로우처럼 무의미한 고정 조합을 피함 | 박스 3필드 + 글로우 3필드, 자막 스키마와 같은 이름 |
| 글로우 기본색 흰색 → **검정** | 1차 미리보기 캡처에서 흰 글자 위 흰 글로우가 묻힘(§4.1) | 켜자마자 소프트 섀도로 가독성 확보 |
| Q4 독립 토글(배타 아님) | 배타 처리는 사용자가 원한 조합을 막는다 | 동시 사용 가능. 이때 글로우는 판 **안쪽**에서 보인다(§3 문서화된 동작) |
| Q5 `hexToRgba` 공유 파일로 이동 | 복제 금지 | `src/compositions/shared/color.ts` 신설, `SubtitleOverlay`는 import 1줄만 변경 |
| 글로우는 `text-shadow` 2겹 | `filter: drop-shadow`는 박스 사각형까지 발광시키고 요소를 매 프레임 래스터화 | 정지 텍스트라 프레임당 비용 무시 가능 |
| 기본 꺼짐 | 기존 프로젝트 산출물 보존 | 저장된 v2 문서가 외곽선만 있는 라벨로 그대로 파싱됨(유닛으로 고정) |

## 2. 변경 파일

| 파일 | 변경 |
|---|---|
| `src/domain/editor/schema.ts` | `day1LabelStyleSchema`에 6필드 `.default()` |
| `src/domain/editor/constants.ts` | `MAX_LABEL_GLOW_PX = 32` |
| `src/domain/editor/types.ts` | 렌더 프롭 `Day1LabelStyle`에 같은 6필드 |
| `src/domain/editor/project.ts` | 기본값 6개 + `updateDay1LabelStyle` 클램프 2개 |
| `src/compositions/shared/color.ts` | **신규** — `hexToRgba` 공용화(순수 이동) |
| `src/compositions/shared/SubtitleOverlay.tsx` | 위 함수 import로 전환(동작 무변경) |
| `src/compositions/day1/Panel.tsx` | `PanelLabel`에 배경/여백/모서리 + `textShadow` 분기 |
| `src/features/editor/Day1Inspector.tsx` | 라벨 섹션에 토글 2 + 조건부 필드 4 |
| `.gitignore` | `artifacts/day1-label-effects/`(렌더 산출물) |

Plan §3의 "변경 없음" 예측이 그대로 지켜졌다: `projectStore.ts`·`EditorWorkspace.tsx`·
`SplitFrame.tsx`·`QuadFrame.tsx`는 한 줄도 바뀌지 않았다(`onLabelStyle`이 이미
`Partial<labelStyle>`을 통과시키고, 프롭 빌더가 이미 스프레드로 전달했기 때문).

## 3. 문서화된 동작 — 박스와 글로우를 함께 켰을 때

글로우는 판의 배경과 **같은 요소**에 얹히는 `text-shadow`이므로, 박스를 켜면 글로우는
판 안쪽에서만 보인다(판 바깥으로 번지지 않는다). Q4에서 확정한 대로 막지 않았고,
`Panel.tsx`의 주석에 같은 내용을 남겼다. 판 밖으로 번지는 발광이 필요하면 별도
사이클에서 `filter: drop-shadow` 옵션을 논의해야 한다.

## 4. Success Criteria Final Status — Iteration 1: 6/6 (SC5는 §6에서 마감)

| SC | 내용 | 상태 |
|----|------|------|
| SC1 | 기존 v2 문서가 6필드 기본값으로 파싱 · 범위 위반 거부 | ✅ 유닛 3건 신규 |
| SC2 | 커맨드 패치·클램프, day1·quad 양쪽 | ✅ 유닛 2건 신규 |
| SC3 | 프롭 통과, day1·quad 양쪽 | ✅ 유닛 2건 신규 |
| SC4 | 토글이 꺼졌으면 하위 필드 비노출, 켜면 노출(두 템플릿) | ✅ e2e L2 2건 신규, 실브라우저 통과 |
| SC5 | 실렌더 MP4에서 판 색·글로우 픽셀 확인 | ✅ **충족** — 배포본에서 사용자가 뽑은 실제 산출물로 확인(§6) |
| SC6 | `tsc -b` + 전체 유닛 + 기존 e2e 그린 | ✅ 유닛 588 · build 성공 · 업로드 불필요한 기존 e2e 7건 통과 |

### 4.1 미리보기 실캡처 — 여섯 조합 시각 확인 (완료)

번들 Chromium이 디코딩할 수 있는 VP8 WebM을 패널 소스로 넣으면 앱의 미리보기
(Remotion Player)가 정상 동작한다. **미리보기와 최종 렌더는 같은 컴포지션**이므로,
이 캡처는 `PanelLabel`이 의도한 픽셀을 그린다는 증거다. 확인한 조합:

| # | 설정 | 결과 |
|---|---|---|
| 1 | 둘 다 꺼짐 | 기존과 동일 — 외곽선만. 회귀 없음 |
| 2 | 박스 `#000000` 60% | 반투명 판이 글자를 감싼다. 60%라 배경색이 비쳐 판이 살짝 물든다 |
| 3 | 글로우 `#000000` 16px (기본값) | 검정 외곽선 **바깥**으로 부드러운 그림자가 번져 글자가 배경에서 떠오른다 |
| 4 | 둘 다 기본값 | 글로우가 판 안쪽에서 보인다 — §3의 문서화된 동작 그대로 |
| 5 | 글로우 `#000000` 32px | 세기를 올리면 그림자가 더 두꺼워진다 |
| 6 | 글로우 `#00E5FF` 32px | 색을 바꾸면 네온 헤일로도 그대로 가능 |
| 7 | 박스 `#1D4ED8` 85% + 글로우 `#00E5FF` 32px | 브랜드 컬러 판 + 시안 글로우 조합 |

인스펙터도 함께 캡처해 토글 2개와 조건부 필드 4개가 의도대로 나타남을 확인했다.

**4분할(Day1(4 video))도 같은 방식으로 캡처했다.** 네 패널 소스를 올리고 같은 다섯 조합을
확인한 결과, 기본 라벨 크기 44px에서도 판이 셀 안에 여유 있게 들어가고 글로우가 셀 경계를
넘지 않는다(패널이 `overflow: hidden`). 라벨 스타일 1세트가 네 셀에 동일하게 적용되는 것도
화면으로 확인 — FR-05가 코드(유닛)와 화면 양쪽에서 고정됐다.

**1차 캡처에서 발견해 고친 것**: 처음 기본 글로우는 흰색 16px이었는데, 흰 글자 위에서는
헤일로가 글자와 섞여 존재감이 거의 없었다. 사용자 확정으로 **기본색을 검정(`#000000`)으로
변경**했다 — 흰 글자 기준 소프트 섀도로 읽혀, 색을 건드리지 않아도 켜자마자 가독성이 오른다.
박스와 함께 켰을 때도 판 안쪽에 흰 헤일로가 갇히던 모습이 사라지고 판이 은은히 깊어지는
쪽으로 개선됐다. 위 표는 변경 후 재캡처 결과다.

### 4.2 SC5(인코딩된 MP4 픽셀)가 미실행인 이유 (환경 제약, 코드 문제 아님)

이 세션의 컨테이너에는 Playwright 번들 Chromium만 있고 `channel: 'chrome'`
(실제 Google Chrome)이 없다. 오픈소스 Chromium 빌드에는 독점 코덱이 없어
**H.264를 디코딩·인코딩하지 못한다.** 실측한 증상:

- 번들 Chromium으로 픽스처(H.264 MP4)를 업로드하면 앱이 정상적으로 거부한다 —
  "Chrome이 이 영상을 열지 못했습니다 (H.264 (avc1))". 업로드가 막히므로 렌더는
  물론 미리보기 DOM 확인도 불가.
- VP8 WebM 소스로 우회해도 프리뷰 게이트(`panelA.url === null`)를 넘지 못했고,
  출력이 H.264이므로 인코딩 단계에서 다시 막힌다.
- `npx playwright install chrome`은 이 환경의 프록시가 차단한다(403).

같은 제약이 기존 스펙에도 그대로 걸린다(업로드가 필요한 기존 Day1 e2e도 이 컨테이너에서는
실행 불가). 따라서 SC5는 **실제 Chrome이 있는 개발/CI 환경에서 `npx playwright test
day1-label-effects.spec.ts`로 확인**해야 한다.

### 4.3 SC5 스펙 설계 — 기하 가정 없이 프레임 전체를 센다

라벨 판의 위치는 시스템 폰트 메트릭에 좌우되므로, 좁은 밴드를 좌표로 찍는 대신
**1080×1920 프레임 전체에서 효과 픽셀 수를 센다**. 0.5s에서 패널 A는 `#e6194b`
컬러로 재생 중이고 패널 B는 흑백 정지이므로, 판 색(`#ff00ff`)이나 초록 우세 픽셀은
footage에서 나올 수 없다. 두 렌더가 서로의 음성(negative)을 증명한다:

| 렌더 | 켠 효과 | 기대 |
|---|---|---|
| `label-box.mp4` | 박스 `#ff00ff` 100% | 판 색 픽셀 > 5,000 **그리고** 초록 우세 < 2,000 (글로우 꺼짐 증명) |
| `label-glow.mp4` | 글로우 `#00ff00` 32px | 초록 우세 > 2,000 **그리고** 판 색 < 5,000 (박스 꺼짐 증명) |

## 5. 병합 전 회귀 검증 — main 대조 실행 (완료)

브라우저가 H.264를 다루지 못해 렌더 계열 E2E가 실패하는 환경이므로, "실패가 이 변경 탓인가"를
눈으로 분류하는 대신 **같은 조건에서 `origin/main`을 돌려 실패 집합을 대조**했다.

| 조건 | 내용 |
|---|---|
| 브라우저 | Playwright 번들 Chromium (WebCodecs 실측: `avc1` encode=false·decode=false, VP9·VP8·AV1=true) |
| 픽스처 | 동일한 VP9-in-MP4 세트(양쪽이 같은 파일을 심볼릭 링크로 공유) |
| 실행 | 브랜치는 포트 4173, main은 별도 worktree + 포트 4183 — 서로의 dev 서버를 재사용하지 않음 |

| | 브랜치 | main |
|---|---|---|
| 전체 스위트(브랜치) | 48 passed / 36 failed | — |
| 대조 15개 스펙 | 34 failed | 34 failed |
| **한쪽에만 있는 실패** | **0건** | **0건** |

실패 34건은 양쪽에서 동일했다. 원인 분류:

| 원인 | 건수 |
|---|---|
| `MP4 렌더` 버튼 disabled / 클릭·`download` 타임아웃 — H.264 인코딩 불가 | 30 |
| `pages-subpath` — 로컬 설정이 프로덕션 빌드 서버(4190)를 띄우지 않음 | 2 |
| `media-codec-compat` — 코덱 자체를 검증하는 스펙(AV1 픽스처는 이 환경 ffmpeg에 `libsvtav1`이 없어 생성 실패) | 2 |

라벨·`labelStyle`·글로우·박스와 관련된 실패는 0건이고, 통과한 48건에는 렌더가 필요 없는 UI 표면
전체(인스펙터·타임라인·템플릿 전환·4분할 UI·트림 UX)와 이번 사이클의 신규 L2 2건이 포함된다.

**코드 차원의 무회귀 근거**도 함께 남긴다: 두 효과가 꺼져 있을 때 라벨 `<span>`이 받는 CSS는
변경 전과 동일하다 — `backgroundColor: 'transparent'`(원래 미지정 = transparent),
`padding: 0`(인라인 요소 기본값), `textShadow: undefined`(React가 속성을 생략),
`borderRadius`는 배경이 없으면 그려지지 않는다.

## 6. 배포본 실렌더 검증 — 사용자 산출물 (SC5 마감)

배포된 사이트(https://sungkk1m.github.io/mkt_Videodesigner/)에서 사용자가 라벨 효과를 켜고
뽑은 실제 MP4 `uavideo_day1x4_ko_9x16_30s_60fps.mp4`를 받아 검증했다. **이 컨테이너에서
불가능했던 SC5가 실제 운영 산출물로 마감됐다.**

| 검사 | 결과 |
|---|---|
| 컨테이너·코덱 | `h264` + `aac`, MP4 — 파일명과 일치 |
| 해상도·프레임레이트·길이 | 1080×1920 · 60fps · 30.017s · 1801 프레임 — `day1x4_ko_9x16_30s_60fps`와 일치 |
| 4분할 레이아웃 | 2×2, 0.5초 시점에 패널 A만 컬러·나머지 셋 흑백 정지 |
| 라벨 문구 | 네 셀에 `1st Wave` / `3rd Wave` / `5th Wave` / `Final Wave` |
| **배경 박스** | 네 셀 모두에 둥근 모서리 반투명 판. 게임 UI(`7.25K`)와 스프라이트가 판 **너머로 비쳐 보임** = 알파 합성이 인코딩까지 살아남음 |
| **글로우** | 검정 외곽선 **바깥**으로 밝은 헤일로가 약 35px에 걸쳐 매끄럽게 감쇠 (사용자가 밝은 색을 고름) |

측정값:

- 패널 A, 판 하단 안쪽(y 98–106) 평균 `(42.8, 41.9, 42.0)` vs 판 바로 아래(y 112–119)
  `(75.9, 63.2, 63.4)` → 경계에서 명확한 단차, 안/밖 비 0.56–0.66
- 패널 D, 판 좌측 경계 가로 프로파일: 바깥 평균 101.6 vs 안쪽 66.0 → 비 0.649
  (두 패널이 독립적인 배경 위에서 같은 감쇠율을 보임 = 판의 알파)
- 글자 주변 세로 프로파일: 외곽선(0) → 85 → 76 → 66 → 55 → 45 → 34로 단조 감쇠,
  전 구간 무채색(r=g=b) → 블러 헤일로의 전형적 프로파일

판의 정확한 불투명도는 헤일로가 판 안쪽을 밝히기 때문에 역산이 깔끔하지 않아 비율로만 보고한다.
**두 효과 모두 H.264 인코딩을 통과해 산출물에 남는다는 것이 이 사이클의 마지막 미검증
항목이었고, 이로써 해소됐다.**

## 7. Iteration 2 — 텍스트 박스 글로우

사용자 추가 요청("텍스트 박스에도 글로우를, 텍스트와 같은 UI로, 단 동기화되지 않게")을
Plan §7-2로 정리해 구현했다.

| 항목 | 내용 |
|---|---|
| 스키마 | `boxGlowEnabled`/`boxGlowColor`/`boxGlowStrengthPx` 3필드, 전부 `.default()` — 마이그레이션 0줄 유지 |
| 렌더 | `Panel.tsx`에서 `box-shadow` 2겹. 글자 글로우와 `halo()` 헬퍼만 공유하고 **인자는 각자의 필드** |
| 게이트 | 렌더·UI 모두 `showBackground` 종속. 박스를 끄면 헤일로도 사라지고 설정값은 저장된 채 남는다 |
| UI | 배경 박스 블록 안에 토글 + 색 + 세기(글자 글로우와 같은 3컨트롤 구성). 기존 "글로우" 라벨은 "글자 글로우"로 명확화 |
| 비동기화 검증 | 커맨드 unit에서 한쪽만 패치했을 때 다른 쪽 불변을 단언하고, 미리보기 캡처로 판=시안·글자=핑크 동시 적용을 확인 |

검증: 유닛 **592 passed**(신규 4) · `tsc -b` + `vite build` · e2e L2 2건 통과 · 미리보기 5개
조합 실캡처. L3 2건은 박스 글로우와 게이트("판을 끄면 헤일로도 없다")까지 포함하도록
확장했으나 이 컨테이너에서는 여전히 실행 불가(§4.2) — 배포 후 사용자 산출물로 확인하는
것이 §6과 같은 경로다.

### 7.1 Iteration 2 회귀 검증 — 전체 E2E 재실행

| 실행 | 결과 |
|---|---|
| Iteration 1 브랜치 (기준선) | 48 passed / 36 failed (31.7분) |
| Iteration 2 (박스 글로우) | 47 passed / **37 failed** (36.1분) |
| 차이 | `hook-analysis › keeps the manual Hook range usable` 1건 |

그 1건은 회귀가 아니다. 근거:

- **단독 재실행에서 3.4초에 통과.** 2워커 병렬 부하(이번 실행이 4.4분 더 느렸다) 아래에서만
  실패했다.
- 실패 원인은 `hook-manual-range`가 `disabled` 상태로 남은 것이고, 그 조건은
  `ready = sourceUrl !== null && (sourceDurationMs ?? 0) > 0`
  (`HookCandidateDrawer.tsx:47`) — **소스 URL과 프로브된 길이**에만 의존한다. 이번 변경은
  `labelStyle` 필드·Day1 라벨 CSS·Day1 인스펙터만 건드린다.
- 이 테스트는 **3장면 템플릿** 경로라 `PanelLabel`을 아예 렌더하지 않는다.
- 스펙 자체가 업로드 직후(파일명만 확인하고) 슬라이더를 조작하는 타이밍 경합 구조이고,
  이 컨테이너에서는 픽스처를 VP9로 치환해 프로브 경로가 원본과 다르다.

나머지 36건은 Iteration 1과 **동일한 집합**이다(환경 기인 34건 + 이 사이클 신규 스펙의
L3 렌더 2건).

## 8. Residual Risks / Follow-ups

| 항목 | 상태 |
|---|---|
| SC5 실렌더 | ✅ 해소 — 배포본 산출물로 확인(§6). L3 스펙 자체는 여전히 실 Chrome이 필요하고, 임계값은 첫 실행에서 실측치를 보고 조정할 수 있다 |
| main 병합 | Iteration 1은 `86409dd`로 병합·자동 배포 완료. Iteration 2는 사용자 확인 대기 |
| README 기능표 | 병합 시점에 라벨 효과 1줄 추가 |
| 판 여백·모서리 노출 | Non-goal 유지(자막과 같은 고정값). 요청이 생기면 별도 사이클 |
