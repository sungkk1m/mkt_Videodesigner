...
| 3 | 글로우 `#FFFFFF` 16px | 검정 외곽선 **바깥**으로 흰 헤일로가 번진다(의도한 방향) |
| 4 | 둘 다 기본값 | 글로우가 판 안쪽에서 보인다 — §3의 문서화된 동작 그대로 |
| 5 | 글로우 `#00E5FF` 32px | 색·세기를 올리면 뚜렷한 네온 헤일로 |
| 6 | 박스 `#1D4ED8` 85% + 글로우 `#00E5FF` 32px | 브랜드 컬러 판 + 시안 글로우 조합 |

인스펙터도 함께 캡처해 토글 2개와 조건부 필드 4개가 의도대로 나타남을 확인했다.

관찰 사항 하나: 기본 글로우(흰색 16px)는 흰 글자 위에서는 존재감이 약하다. 실사용에서는
색을 바꾸거나 세기를 올리는 쪽이 효과적이다 — 기본값 변경 여부는 사용자 판단에 맡긴다.
