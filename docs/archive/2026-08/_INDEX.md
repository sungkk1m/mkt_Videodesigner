...
| [day1-endcard-audio](day1-endcard-audio/) | #8 | **100%** | 2026-08-17 (단일 세션) | Plan(경량 설계 내장) · Check 분석 · 완료 리포트 |

## day1-endcard-audio

엔드카드 영상의 **자기 오디오를 살렸다** — endcard-video FR-05의 "항상 무음"
결정을 뒤집어, 영상 모드에 토글+볼륨(기본 켜짐 100%)을 두고 선택한 트림 구간의
소리가 영상 루프와 함께 재생되게 했다. 15초에서 루프가 잘릴 때를 대비한
0.25초 종료 페이드아웃 포함. 배너 모드·패널·BGM 경로는 무변경.
...
| Iteration | 0 |
| 최종 검증 | 유닛 371테스트 · E2E 56 passed(2 opt-in skip) · `tsc -b` · 실렌더 오디오 측정 3종 · 라이브 스모크 7/7 |
| 착수 시 기준선 | 유닛 363테스트 · E2E 53 passed |

**시작점**: [완료 리포트](day1-endcard-audio/day1-endcard-audio.report.md) — 검증
질문에서 사이클이 시작된 경위와 Key Decisions가 여기 있습니다.

### 이 사이클에서 남길 만한 것

**검증 질문이 사이클의 가장 좋은 입구다.** "엔드카드 오디오가 사라지는 게
맞나요?"에 코드만 보고 답할 수도 있었지만, 설계 문서(FR-05) · 코드(`muted`) ·
실산출물(13–15s −91dB) 3면을 대조한 덕분에 의도된 결정임을 근거와 함께 제시하고,
뒤집을 때 건드릴 지점이 이미 파악된 상태로 Plan에 들어갔다.

**실렌더 테스트가 배포 후 발견될 버그를 잡았다.** `@remotion/media`는 volume
콜백을 `frame=NaN`으로 프로브하고 **NaN을 돌려받으면 렌더 전체를 거부한다**.
유닛도 타입도 전부 통과한 상태였고, L3 실렌더 e2e만이 이것을 잡았다. 기존
`duckedVolumeAt`이 안전했던 이유(NaN 비교가 false라 base로 폴백)를 확인해 같은
구조로 정렬했다.

**측정 임계값은 실측 프로파일을 보고 정한다.** 페이드 어서션의 첫 실패는 기능이
아니라 측정창 선택 문제였다(4.7dB 드롭 < 임계 5dB). 구간별 프로파일(−21.1 →
−24.7 → −30.1 → −33.6dB)을 먼저 뽑고 나서야 견고한 창을 고를 수 있었다.

**네이티브 기능이 또 한 번 설계를 줄였다.** 별도 `<Audio>` 트랙 없이 Video의
`muted`만 풀면 `@remotion/media`가 트림·루프와 오디오를 알아서 동기화한다 —
동기화 코드 0줄. (endcard-video 사이클의 `loop` 발견과 같은 패턴.)
