...
> 상태: **Iteration 1 완료·main 병합·배포 완료(2026-08-26, `86409dd`)** · **Iteration 2(박스 글로우) 구현 완료**
...
## 7-2. Iteration 2 — 텍스트 박스 글로우 (2026-08-26 추가 요청)

Iteration 1 배포 후 사용자 요청: "텍스트 박스에도 글로우 효과를 넣어달라. **텍스트와 동일한
UI**를 적용하되, **텍스트와 동기화되지는 않도록**(박스 글로우를 흰색으로 해도 글자 글로우가
흰색이 되지 않게)."

### 요구사항

- **FR-07 박스 글로우 on/off**: 배경 박스가 켜진 동안에만 "박스 글로우" 토글이 노출된다.
  켜면 **판의 사각형 둘레**에 발광이 번진다(`box-shadow`). 기본 꺼짐.
- **FR-08 독립 값**: 색·세기를 글자 글로우와 **별도 필드**로 갖는다. 어느 한쪽을 바꿔도
  다른 쪽은 변하지 않는다. 기본 `#000000` / 16px — 판을 배경에서 띄우는 소프트 섀도.

### 성공 기준

- **SC7** 커맨드/프롭 unit: 한쪽 글로우만 패치했을 때 다른 쪽 값이 그대로 유지되고,
  두 값이 각각 프롭으로 전달된다(day1·quad 양쪽).
- **SC8** 스키마 unit: 기존 문서가 3필드 기본값으로 파싱되고 세기 범위 위반은 거부된다.
- **SC9** e2e(L2): 박스가 꺼져 있으면 박스 글로우 컨트롤 자체가 없고, 박스를 켜면 나타나며,
  글자 글로우 컨트롤을 끌어들이지 않는다. 박스를 다시 끄면 함께 사라진다.
- **SC10** e2e(L3): 박스 렌더에 판 헤일로 픽셀이 존재하고, 글자 글로우만 켠 렌더에는
  (박스 글로우를 켠 뒤 박스를 껐으므로) 헤일로 픽셀이 **없다** — 게이트까지 산출물로 고정.

### 설계 결정

| 결정 | 근거 |
|---|---|
| `box-shadow` 2겹 (`halo()` 헬퍼를 글자 글로우와 공유) | 판의 사각형에 번지는 유일한 CSS 수단. 함수는 공유하되 **인자는 각자의 필드** — 코드 중복 없이 값은 분리 |
| 신규 3필드 `boxGlow*` (기존 `glow*` 재사용 안 함) | FR-08의 비동기화 요구가 곧 "필드를 공유하지 않는다"는 뜻 |
| 렌더·UI 모두 `showBackground`에 종속 | 판이 없는 상태의 헤일로는 보이지 않는 사각형이 빛나는 결과가 된다. 설정값은 저장된 채 숨는다(엔드카드 모드와 같은 패턴) |
| 인스펙터 라벨을 "글로우" → "글자 글로우"로 명확화 | 글로우가 둘이 되면서 기존 라벨이 모호해졌다. testid는 그대로 |
