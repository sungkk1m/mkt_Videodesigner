...
> 상태: **Plan 확정 대기** — §7 Checkpoint 5문항 사용자 확답 후 Do 진입
...
## 7. Checkpoint — 확답이 필요한 5건 (권장안 표기)

| # | 질문 | 권장 |
|---|---|---|
| Q1 | "텍스트 박스"는 (a) 글자 뒤 **반투명 채움 판**인가, (b) 글자를 두르는 **테두리 사각형**인가? | **(a)** — 자막 "배경 사용"과 같은 개념이고 가독성 목적에 직접 부합 |
| Q2 | 박스는 on/off만 둘까, 색·불투명도까지 열까? | **색·불투명도까지** — 자막과 동일 구성(3필드). 다크 판 하나로 고정하면 밝은 톤 소재에서 못 쓴다 |
| Q3 | 글로우는 on/off만 둘까, 색·세기까지 열까? | **색·세기까지** — 기본 흰 글자에 흰 글로우 고정이면 소재에 따라 안 보인다 |
| Q4 | 박스와 글로우 **동시** 사용 시 글로우가 판 안쪽에만 보이는 동작을 허용할까, 아니면 배타(하나 켜면 하나 꺼짐)로 둘까? | **허용(독립 토글)** — 배타 처리는 사용자가 원한 조합을 막는다. 대신 Report에 동작을 명기 |
| Q5 | `hexToRgba`를 `src/compositions/shared/color.ts`로 순수 이동하고 자막이 그것을 import하게 할까(1줄 변경), 복제할까? | **이동** — 복제 금지 원칙, 아키텍처 테스트 영향 없음 |

기본값도 함께 확인해 주세요: 박스 `#000000` / 60%, 글로우 `#ffffff` / 16px, **둘 다 기본 꺼짐**.
